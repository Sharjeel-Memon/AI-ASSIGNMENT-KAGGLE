...
RUN_KFOLD_FOR_BEST_MODEL = False
...
# ITERATION 2: This script will force a tuned gradient boosting model for the Kaggle submission.
# This helps compare against Iteration 1 Decision Tree results.
# Set to None if you want the script to automatically choose the validation winner again.
FORCE_FINAL_MODEL_NAME = "HistGradientBoosting Tuned V2"
...
# Use a non-GUI plotting backend so Ubuntu/Tkinter does not crash while saving PNGs.
import matplotlib
matplotlib.use("Agg")
...
output_dir = Path("outputs_iter2_safe")
...
models = {
    ...
    "HistGradientBoosting Tuned V2": Pipeline(steps=[
        ("preprocess", ordinal_preprocess),
        ("model", HistGradientBoostingClassifier(
            learning_rate=0.045,
            max_iter=700,
...
auto_best_model_name = results_df.iloc[0]["model_name"]
auto_best_validation_score = results_df.iloc[0]["balanced_accuracy"]

if FORCE_FINAL_MODEL_NAME is not None:
    if FORCE_FINAL_MODEL_NAME not in models:
        raise ValueError(f"FORCE_FINAL_MODEL_NAME was set to {FORCE_FINAL_MODEL_NAME!r}, but that model does not exist.")
    best_model_name = FORCE_FINAL_MODEL_NAME
else:
    best_model_name = auto_best_model_name

best_validation_score = float(
    results_df.loc[results_df["model_name"] == best_model_name, "balanced_accuracy"].iloc[0]
)

print("\nAuto best validation model:", auto_best_model_name)
print("Auto best validation balanced accuracy:", auto_best_validation_score)
print("Final Kaggle submission model:", best_model_name)
print("Final model validation balanced accuracy:", best_validation_score)
...
with open(report_notes, "w", encoding="utf-8") as f:
    ...
    if FORCE_FINAL_MODEL_NAME is not None:
        f.write("Reason selected: Iteration 2 intentionally uses a tuned HistGradientBoosting model to compare Kaggle generalization against the Iteration 1 Decision Tree.\n\n")
    else:
...
print("2. outputs_iter2_safe/submission.csv")
